...
{{-- Tambahan Font Awesome untuk Icon agar lebih profesional --}}
...
<div class="admin-wrapper" style="padding: 20px; background: #f8fafc;">
    ...
    <div class="detail-wrapper" style="background: white; padding: 25px; border-radius: 15px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1);">
        ...
        <table class="table table-bordered align-middle">
            ...
            <tr>
                ...
                <td class="font-monospace">{{ $email->nomor_identitas }}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td><i class="fa fa-envelope me-1 text-muted"></i> {{ $email->email_alternatif }}</td>
            </tr>
            <tr>
    ...
    @if($email->status == 'pending')
        ...
        <div id="sectionSetuju" class="mt-4 p-4" style="background: #ecfdf5; border-radius: 15px; border: 1px solid #10b981;">
            ...
            <form action="{{ route('admin.emailpribadi.setuju', $email->id) }}" method="POST" id="formSetuju">
                @csrf
                ...
                <div class="action-wrapper mt-3 d-flex gap-2">
                    ...
                    {{-- Ubah type menjadi button biasa agar tidak men-trigger submit form setuju --}}
                    <button type="button" class="btn btn-danger px-4 fw-bold shadow-sm" id="btnTolak">
                        <i class="fa fa-times me-1"></i> Tolak Permohonan
                    </button>
                    <a href="{{ route('admin.kelolaemail') }}" class="btn btn-secondary px-4 fw-bold shadow-sm">
        ...
        {{-- Form Penolakan (Diletakkan SEPENUHNYA di luar Form Persetujuan) --}}
        <div id="formTolak" class="mt-4 p-4" style="display:none; background: #fff; border-radius: 15px; border: 1px solid #ef4444;">
            ...
            <form action="{{ route('admin.emailpribadi.tolak', $email->id) }}" method="POST" id="mainFormTolak">
                @csrf
                <input type="hidden" name="id" value="{{ $email->id }}">
    ...
    @else
        {{-- Tampilan detail setelah disetujui atau ditolak --}}
        <div class="detail-wrapper mt-4 p-4" style="background: white; border-radius: 15px; border: 1px solid {{ $email->status == 'disetujui' ? '#10b981' : '#cbd5e1' }}; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
            @if($email->status == 'disetujui')
                    ...
                    <div class="col-md-6">
                        <table class="table table-borderless">
                            <tr>
                                ...
                                <td class="py-1 fw-bold text-primary fs-5">{{ $akunAktif->nama_akun ?? ($email->nama_akun ?? 'belum.aktif') }}@institusi.ac.id</td>
                            </tr>
                            <tr>
                                <th class="small text-secondary text-uppercase py-1">Tanggal Aktivasi</th>
                                <td class="py-1">{{ $email->updated_at->format('d F Y, H:i') }} WIB</td>
                            </tr>
                        </table>
                    </div>
                </div>
            ...
            @else
                ...
                <div style="background: #fff; padding: 5px 10px;">
                    ...
                    <p class="text-muted mb-3" style="font-size: 13.5px;">
                        Diproses pada: {{ $email->updated_at->format('d F Y, H:i') }} WIB
                    </p>
                    
                    <h5 class="mb-1" style="font-weight: 700; color: #000; font-size: 15px;">Alasan Penolakan:</h5>
                    <p class="mb-4" style="color: #000; font-size: 14px;">{{ $email->alasan_tolak }}</p>
                    
                    <div class="mt-2">
                        <a href="{{ route('admin.kelolaemail') }}" class="btn btn-secondary px-4 py-2 fw-bold shadow-sm" style="border-radius: 20px !important;">
